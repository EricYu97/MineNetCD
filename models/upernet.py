...
from transformers import UperNetConfig, AutoBackbone
from .vmamba_single import VSSM_ST_T, VSSM_ST_S, VSSM_ST_B, VSSM_L
...
class UperNetForSemanticSegmentation(UperNetPreTrainedModel):
    def __init__(self, config):
        ...
        if config.Backbone_type=="Swin":
            ...
            config.auxiliary_channels=256
        #Diff
        elif config.Backbone_type=="VSSM_B_ST_Diff":
            pretrained="./pretrained_weights/vssm_base_0229_ckpt_epoch_237.pth"
            self.backbone = VSSM_ST_B(pretrained=pretrained)
            self.backbone_type="diff"
            config.auxiliary_in_channels=self.backbone.dims[2]
            config.auxiliary_channels=256
        elif config.Backbone_type=="VSSM_S_ST_Diff":
            pretrained="./pretrained_weights/vssm_small_0229_ckpt_epoch_222.pth"
            self.backbone = VSSM_ST_S(pretrained=pretrained)
            self.backbone_type="diff"
            config.auxiliary_in_channels=self.backbone.dims[2]
            config.auxiliary_channels=256
        elif config.Backbone_type=="VSSM_T_ST_Diff":
            pretrained="./pretrained_weights/vssm_tiny_0230_ckpt_epoch_262.pth"
            self.backbone = VSSM_ST_T(pretrained=pretrained)
            self.backbone_type="diff"
            config.auxiliary_in_channels=self.backbone.dims[2]
            config.auxiliary_channels=256
    ...
    def forward(
        ...
        if self.backbone_type=="swin":
            ...
            pixel_values[pixel_values.shape[0]//2:,:,:,:], output_hidden_states=False, output_attentions=False).feature_maps
            ...
            outputs=[]
            outputs_A = self.backbone(pixel_values[pixel_values.shape[0]//2:,:,:,:])
            outputs_B = self.backbone(pixel_values[:pixel_values.shape[0]//2,:,:,:])
            for i in range(4):
            ...
            outputs=[]
            outputs_A = self.backbone(pixel_values[pixel_values.shape[0]//2:,:,:,:]).feature_maps
            outputs_B = self.backbone(pixel_values[:pixel_values.shape[0]//2,:,:,:]).feature_maps
            for i in range(4):
                # output=torch.cat([outputs_A[i],outputs_B[i]],dim=1)
                output=outputs_A[i]-outputs_B[i]
                if self.channel_mixing:
            ...
            outputs = self.backbone(pixel_values[pixel_values.shape[0]//2:,:,:,:], pixel_values[:pixel_values.shape[0]//2,:,:,:])
            features = outputs
